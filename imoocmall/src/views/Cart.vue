...
import {currency} from './../util/currency' // 导入带名字的文件
...
export default{
  ...
  methods: {
    ...
    init(){
      axios.get('/users/cartList').then((response) => {
        if(response.data.status==0){ // 如果登陆了
          this.cartList = response.data.result // 接收后台传来的userName数据
        }
      })
    },
    ...
    delCartConfirm(item,productId){
      this.delItem = item
      this.productId = productId;// 保存商品id，为了点击确定删除时有数据
      // 显示弹出层
    ...
    delCart(){
      ...
      axios.post('/users/cartDel',{
        productId:this.productId // 向服务器提交商品id
      }).then((response) => {
        if(response.data.status==0){
          this.modalConfirm = false;// 关闭弹出层
          this.init() // 重更新渲染页面
    ...
    editCart(flag,item){
      ...
      axios.post('/users/cartEdit',{
        productId:item.productId, // 向服务器提交商品id
  ...
  computed:{ 
    // 计算商品是否全被选定(只要cartList的checked=1的项等于购物车商品总数就行)
    checkAllFlag(){ // 时刻计算checkAllFlag
