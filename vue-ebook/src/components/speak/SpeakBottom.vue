<template>
  <div class="bottom-wrapper" :class="{'hide-play': !showPlay}">
    ...
    <div class="bottom-btn-wrapper" @click="addOrRemoveShelf">
      <span class="icon-check" v-if="inBookShelf"></span>
      <span class="bottom-btn-text">{{inBookShelf ? $t('detail.isAddedToShelf') : $t('detail.addOrRemoveShelf')}}</span>
    </div>
  </div>
</template>
...
import { saveBookShelf } from '../../utils/localStorage'
  ...
  export default {
    ...
    methods: {
      ...
      addOrRemoveShelf() {
        if (this.inBookShelf) {
          this.setShelfList(removeFromBookShelf(this.bookItem2)).then(() => {
            saveBookShelf(this.shelfList)
          })
        } else {
          addToShelf(this.bookItem)
          // this.setShelfList(getBookShelf()).then(() => {
          //   alert('ok')
          // })
        }
      }
