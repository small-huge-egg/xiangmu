<template>
    <div class="view" @mouseenter="enter" @mouseleave="leave">
        <div class="btns">
        <div v-for="(x,index) in list" :key=index @click="num=index" :class="{con:index==num}">{{index+1}}</div>
        </div>
            <img @transitionend="end" v-for="(x,index) in list" :key=index :style="{opacity:index==num?1:0}" :src="'../../static/'+x+'.jpg'">
...
export default {
    ...
    methods:{
        next:function(){
            // this.num=this.num>this.list.length-2?0:this.num+1;
            if(this.flag){
                this.flag=false;
                this.num++;
