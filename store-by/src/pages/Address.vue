<template>
    <div>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="next-btn-wrap">
              <router-link class="btn btn--m btn--red" :to="{path:'orderConfirm',query:{'token':$route.query.token}}">去结账</router-link>
            </div>
          </div>
        </div>
      </div>
      ...
      <modal :mdShow="isMdShowLast" @close="closeModal">
        ...
        <div slot="btnGroup">
          <a href="javascript:;" class="btn btn--m" @click="isMdShowLast=false">那我不删了</a>
        </div>
      </modal>
...
export default{
  ...
  methods: {
    init(){
      // axios.get('/users/addressList').then((response) => {
        ...
        //   this.addressList = response.data.result // 接收后台传来的userName数据
        // }
      // })
      // this.setDefault(this.morenId)
    },
    ...
    setDefault(addressId) {
      ...
      this.addressListFilter.forEach((item,index)=>{
        item.isDefault = false
        if(item.addressId==addressId){
          item.isDefault = true
          this.morenId = addressId
        }else{
          item.isDefault = false
        }
      })
    ...
    closeModal(){
      this.isMdShow = false,
      this.isMdShowLast = false
    },

    // 点击删除地址
    delAddress(addressId){ 
      if(this.addressListFilter.length<=1){ // 如果是最后一条数据
    ...
    delAddressConfirm(){
      ...
      this.addressListFilter.forEach((item,index)=>{
        if(this.addressId==item.addressId){
          this.addressListFilter.splice(index,1)
        }
      })
      this.init()
    }
  ...
  computed:{
    // mount(){ // 过滤地址栏地址数量
    //   return this.mount.slice(0,this.limit)
    // }
  },
