  ...
  methods:{
    init(){ // 获取购物车数据
      axios.get('/users/cartList').then((response)=>{
        ...
        this.orderTotal = this.subTotal - this.discount + this.shiping + this.tax
      })
    },

    // 去结算
    payMent(){ 
      let addressId = this.$route.query.addressId // 获取路由参数，将订单地址传给后台，
      axios.post('/users/payMent',{
        addressId: addressId, // 将订单地址传给后台，
        orderTotal: this.orderTotal // 将订单金额传给后台 
      }).then((response)=>{
        if(response.data.status==0){
          console.log("order created suc")
          this.$router.push({ // 导航到ok页面
